...
gSmartTriggerRegister[NUM_OF_SMART_TRIGGER_REGISTRATION];
...
#define MakeFunction(a, b)                        \
  void a##b(const char *outBuf)                   \
  {                                               \
...
// The second parameter of below calls could be one less than
// NUM_OF_SMART_TRIGGER_REGISTRATION i.e. (NUM_OF_SMART_TRIGGER_REGISTRATION -
// 1)
MakeFunction(TriggerCallback, 0) MakeFunction(TriggerCallback, 1)
    MakeFunction(TriggerCallback, 2) MakeFunction(TriggerCallback, 3)
        MakeFunction(TriggerCallback, 4) MakeFunction(TriggerCallback, 5)
            MakeFunction(TriggerCallback, 6) MakeFunction(TriggerCallback, 7)
...
#define PrepareCallbackFunction(a, b) a##b
    ...
    SQLRETURN
    SQLBindFileToParam(SQLHSTMT StatementHandle, // hstmt
                       SQLUSMALLINT TargetType,  // ipar
                       SQLSMALLINT DataType,     // fSqlType
                       SQLCHAR *FileName, SQLSMALLINT *FileNameLength,
                       SQLUINTEGER *FileOptions, SQLSMALLINT MaxFileNameLength,
                       SQLINTEGER *IndicatorValue)
{
  printf("\n Error: SQLBindFileToParam not supported yet");
  printf("\n Binding a parameter marker in an SQL statement to a file "
         "reference will be a new feature");
  return (SQL_ERROR);
}
////////////////////////////////////////////////////////////
...
static void _python_IfxPy_free_conn_struct(conn_handle *handle)
{
  ...
  storeSessionID *rootNode = (storeSessionID *)handle->session_id_node;
  tempNode = rootNode;
  while (tempNode != NULL) {
    delNode = tempNode;
...
{
  if ((handle->hstmt != (SQLHSTMT)-1) &&
      (handle->connhandle->handle_active == 1)) {
...
static void _python_IfxPy_check_sql_errors(SQLHANDLE handle, SQLSMALLINT hType,
                                           int rc, int cpy_to_global,
                                           char *ret_str, int API,
                                           SQLSMALLINT recno)
{
  ...
  if (rc1 == SQL_SUCCESS) {
    ...
    case SQL_ERROR:
      if (cpy_to_global) {
        switch (hType) {
        case SQL_HANDLE_DBC:
          strncpy(IFX_G(__python_conn_err_state), (char *)sqlstate,
                  SQL_SQLSTATE_SIZE + 1);
          strncpy(IFX_G(__python_conn_err_msg), (char *)errMsg,
                  DB_MAX_ERR_MSG_LEN);
          break;

        case SQL_HANDLE_STMT:
          strncpy(IFX_G(__python_stmt_err_state), (char *)sqlstate,
                  SQL_SQLSTATE_SIZE + 1);
          strncpy(IFX_G(__python_stmt_err_msg), (char *)errMsg,
                  DB_MAX_ERR_MSG_LEN);
          break;
        }
      }
    ...
    case SQL_SUCCESS_WITH_INFO:
      ...
      if (cpy_to_global) {
        switch (hType) {
        case SQL_HANDLE_DBC:
          strncpy(IFX_G(__python_conn_warn_state), (char *)sqlstate,
                  SQL_SQLSTATE_SIZE + 1);
          strncpy(IFX_G(__python_conn_warn_msg), (char *)errMsg,
                  DB_MAX_ERR_MSG_LEN);
          break;

        case SQL_HANDLE_STMT:
          strncpy(IFX_G(__python_stmt_warn_state), (char *)sqlstate,
                  SQL_SQLSTATE_SIZE + 1);
          strncpy(IFX_G(__python_stmt_warn_msg), (char *)errMsg,
                  DB_MAX_ERR_MSG_LEN);
          break;
        }
      }
...
#ifdef HAVE_SMARTTRIGGER
static PyObject *IfxPy_join_smart_trigger_session(PyObject *self,
                                                  PyObject *args)
{
  ...
  if (PyArg_ParseTuple(args, "OOOO", &py_conn_res, &callbackObj, &sessionIDObj,
                       &ControlBackToApplicationObj)) {
    ...
    if (!PyLong_Check(sessionIDObj)) {
      PyErr_SetString(PyExc_TypeError,
                      "Error : Session ID object should be of type int/long");
      return NULL;
    }

    if (!PyBool_Check(ControlBackToApplicationObj)) {
      PyErr_SetString(PyExc_TypeError,
                      "Error : Input object should be of type Bool/Boolean");
      return NULL;
    }
    ...
    if (!NIL_P(py_conn_res)) {
      if (!PyObject_TypeCheck(py_conn_res, &conn_handleType)) {
        PyErr_SetString(PyExc_Exception,
                        "Supplied connection object Parameter is invalid");
        return NULL;
      } else {
      ...
      gJoinSmartTrigger.callback = TriggerCallback0;
      gJoinSmartTrigger.joinSessionID = &sessionID;
      gJoinSmartTrigger.ControlBackToApplication = &ControlBackToApplication;

      rc = SQLSetStmtAttr((SQLHSTMT)tmpHstmt, PY_IFMX_JOIN_SMART_TRIGGER,
                          &gJoinSmartTrigger, SQL_NTS);
      if (rc == SQL_ERROR) {
        _python_IfxPy_check_sql_errors(conn_res->hdbc, SQL_HANDLE_DBC, rc, 1,
                                       NULL, -1, 1);
        PyMem_Del(return_str);
...
static PyObject *IfxPy_delete_smart_trigger_session(PyObject *self,
                                                    PyObject *args)
{
  PyObject *result = NULL;
  PyObject *py_conn_res = NULL;
  ...
  if (PyArg_ParseTuple(args, "OO", &py_conn_res, &sessionIDObj)) {
    ...
    if (!NIL_P(py_conn_res)) {
      ...
      rc = SQLSetStmtAttr((SQLHSTMT)tmpHstmt, PY_IFMX_DELETE_SMART_TRIGGER,
                          &sessionID, SQL_NTS);
      if (rc == SQL_ERROR) {
        _python_IfxPy_check_sql_errors(conn_res->hdbc, SQL_HANDLE_DBC, rc, 1,
                                       NULL, -1, 1);
...
static PyObject *IfxPy_get_smart_trigger_sessionID(PyObject *self,
                                                   PyObject *args)
{
  PyObject *result = NULL;
  PyObject *sessionIDObj = NULL;
    ...
    if (sessionIDObj != NULL && sessionIDObj != Py_None) {
      ...
      if (found == FALSE) {
        PyErr_SetString(PyExc_Exception, "The associated session ID not found "
                                         "for the unique string provided.");
        return NULL;
      }
    } else {
      PyErr_SetString(
          PyExc_Exception,
          "Argument is NULL or Py_None. It should be non-Null and should "
          "contain proper value of length not more than 128 bytes.");
      return NULL;
    }
  }
  return result;
...
static PyObject *IfxPy_open_smart_trigger(PyObject *self, PyObject *args)
{
  ...
  int rc;
  char *return_str =
      NULL; // This variable is used by _python_IfxPy_check_sql_errors to return
            // err strings
  ...
  if (PyArg_ParseTuple(args, "OOOOOO|O", &py_conn_res,
                       &uniqueSessionIDStringObj, &isDetachableObj, &timeOutObj,
                       &maxRecsPerReadObj, &maxPendingOperationsObj,
                       &reservedObj)) {
    if (!PyBool_Check(isDetachableObj) || !PyLong_Check(timeOutObj) ||
        !PyLong_Check(maxRecsPerReadObj) ||
        !PyLong_Check(maxPendingOperationsObj)) {
      PyErr_SetString(PyExc_TypeError, "One of the paremeters is not of "
                                       "correct type. Recheck and try again.");
      return NULL;
    }

    if (uniqueSessionIDStringObj != NULL &&
        uniqueSessionIDStringObj != Py_None) {
      if (PyString_Check(uniqueSessionIDStringObj) ||
          PyUnicode_Check(uniqueSessionIDStringObj)) {
        uniqueSessionIDStringObj =
            PyUnicode_FromObject(uniqueSessionIDStringObj);
      } else {
        PyErr_SetString(PyExc_Exception,
                        "Argument must be a string or unicode");
        return NULL;
      ...
      uniqueSessionIDStringObj =
          PyUnicode_AsASCIIString(uniqueSessionIDStringObj);
      if (uniqueSessionIDStringObj == NULL) {
        PyErr_SetString(PyExc_Exception,
                        "Argument must be a string or unicode");
        return NULL;
    ...
    if (!NIL_P(py_conn_res)) {
      ...
      rc = SQLSetStmtAttr((SQLHSTMT)tmpHstmt, PY_IFMX_OPEN_SMART_TRIGGER,
                          &gopenSmartTrigger, SQL_NTS);
      if (rc == SQL_ERROR) {
        _python_IfxPy_check_sql_errors(conn_res->hdbc, SQL_HANDLE_DBC, rc, 1,
                                       NULL, -1, 1);
        PyMem_Del(return_str);
      ...
      rc = SQLGetStmtAttr((SQLHSTMT)tmpHstmt,
                          PY_IFMX_GET_LO_FILE_DESC_SMART_TRIGGER,
                          (int *)&fileDesc, SQL_NTS, NULL);
      if (rc == SQL_ERROR) {
        _python_IfxPy_check_sql_errors(conn_res->hdbc, SQL_HANDLE_DBC, rc, 1,
                                       NULL, -1, 1);
        PyMem_Del(return_str);
      ...
      rc = SQLGetStmtAttr((SQLHSTMT)tmpHstmt,
                          PY_IFMX_GET_SESSION_ID_SMART_TRIGGER,
                          (int *)&getSesId, SQL_NTS, NULL);
      if (rc == SQL_ERROR) {
        _python_IfxPy_check_sql_errors(conn_res->hdbc, SQL_HANDLE_DBC, rc, 1,
                                       NULL, -1, 1);
        PyMem_Del(return_str);
...
static PyObject *_python_IfxPy_register_smart_trigger_helper(PyObject *self,
                                                             PyObject *args,
                                                             BOOL isLoop)
{
  ...
  if (PyArg_ParseTuple(args, "OOOOOOOOOO|O", &py_conn_res, &callbackObj,
                       &loFileDescriptorObj, &tableNameObj, &ownerNameObj,
                       &dbNameObj, &sqlQueryObj, &labelObj, &isDeregisterObj,
                       &ControlBackToApplicationObj, &reservedObj)) {
    if (!PyCallable_Check(callbackObj)) {
      PyErr_SetString(PyExc_TypeError, "parameter must be callable");
      return NULL;
    }
    ...
    if (Counter >= NUM_OF_SMART_TRIGGER_REGISTRATION) {
      PyErr_SetString(
          PyExc_Exception,
          "Can't register more than 10 events. Increase the counter (and "
          "modify associated code) to suit your need in the Python code.");
      return NULL;
    }
    ...
    if (!PyLong_Check(loFileDescriptorObj)) {
      PyErr_SetString(PyExc_Exception,
                      "File Description parametr should be integer");
      return NULL;
    }
    if (!PyBool_Check(isDeregisterObj) ||
        !PyBool_Check(ControlBackToApplicationObj)) {
      PyErr_SetString(PyExc_TypeError, "One of the paremeters is not of "
                                       "boolean type. Recheck and try again.");
      return NULL;
    }
    if (tableNameObj != NULL && tableNameObj != Py_None) {
      if (PyString_Check(tableNameObj) || PyUnicode_Check(tableNameObj)) {
        ...
        PyErr_SetString(PyExc_Exception,
                        "statement must be a string or unicode");
        return NULL;
      }

      objectSize = PyUnicode_GET_DATA_SIZE(tableNameObj);
      tableName = (SQLWCHAR *)malloc((int)objectSize * sizeof(SQLWCHAR) +
                                     sizeof(SQLWCHAR));
      if (tableName == NULL) {
        PyErr_SetString(PyExc_Exception,
                        "Table name : Memory allocation failed.");
        return NULL;
      }
      ...
      if ((int)retLength < 0) {
        PyErr_SetString(
            PyExc_Exception,
            "Unicode to wide conversion failed for Table name object.");
        return NULL;
      }
    }
    if (ownerNameObj != NULL && ownerNameObj != Py_None) {
      ...
      ownerName = (SQLWCHAR *)malloc((int)objectSize * sizeof(SQLWCHAR) +
                                     sizeof(SQLWCHAR));
      if (ownerName == NULL) {
        PyErr_SetString(PyExc_Exception,
                        "Owner name : Memory allocation failed.");
        return NULL;
      }
      ...
      if ((int)retLength < 0) {
        PyErr_SetString(
            PyExc_Exception,
            "Unicode to wide conversion failed for Owner name object.");
        return NULL;
      }
    }
    if (dbNameObj != NULL && dbNameObj != Py_None) {
      ...
      dbName = (SQLWCHAR *)malloc((int)objectSize * sizeof(SQLWCHAR) +
                                  sizeof(SQLWCHAR));
      if (dbName == NULL) {
      ...
      if ((int)retLength < 0) {
        PyErr_SetString(
            PyExc_Exception,
            "Unicode to wide conversion failed for Database name object.");
        return NULL;
      }
    }
    if (sqlQueryObj != NULL && sqlQueryObj != Py_None) {
      ...
      sqlQuery = (SQLWCHAR *)malloc((int)objectSize * sizeof(SQLWCHAR) +
                                    sizeof(SQLWCHAR));
      if (sqlQuery == NULL) {
        PyErr_SetString(PyExc_Exception,
                        "SQL Query : Memory allocation failed.");
        return NULL;
      }
      ...
      if ((int)retLength < 0) {
        PyErr_SetString(
            PyExc_Exception,
            "Unicode to wide conversion failed for SQL Query object.");
        return NULL;
      }
    }
    if (labelObj != NULL && labelObj != Py_None) {
      ...
      label = (SQLWCHAR *)malloc((int)objectSize * sizeof(SQLWCHAR) +
                                 sizeof(SQLWCHAR));
      if (label == NULL) {
      ...
      if ((int)retLength < 0) {
        PyErr_SetString(PyExc_Exception,
                        "Unicode to wide conversion failed for Label object.");
        return NULL;
      }
    }
    ...
    if (!NIL_P(py_conn_res)) {
      ...
      switch (Counter) {
      case 0:
        gSmartTriggerRegister[Counter].callback =
            PrepareCallbackFunction(TriggerCallback, 0);
        break;
      case 1:
        gSmartTriggerRegister[Counter].callback =
            PrepareCallbackFunction(TriggerCallback, 1);
        break;
      case 2:
        gSmartTriggerRegister[Counter].callback =
            PrepareCallbackFunction(TriggerCallback, 2);
        break;
      case 3:
        gSmartTriggerRegister[Counter].callback =
            PrepareCallbackFunction(TriggerCallback, 3);
        break;
      case 4:
        gSmartTriggerRegister[Counter].callback =
            PrepareCallbackFunction(TriggerCallback, 4);
        break;
      case 5:
        gSmartTriggerRegister[Counter].callback =
            PrepareCallbackFunction(TriggerCallback, 5);
        break;
      case 6:
        gSmartTriggerRegister[Counter].callback =
            PrepareCallbackFunction(TriggerCallback, 6);
        break;
      case 7:
        gSmartTriggerRegister[Counter].callback =
            PrepareCallbackFunction(TriggerCallback, 7);
        break;
      case 8:
        gSmartTriggerRegister[Counter].callback =
            PrepareCallbackFunction(TriggerCallback, 8);
        break;
      case 9:
        gSmartTriggerRegister[Counter].callback =
            PrepareCallbackFunction(TriggerCallback, 9);
        break;
      default:
        PyErr_SetString(
            PyExc_Exception,
      ...
      gSmartTriggerRegister[Counter].ControlBackToApplication =
          &ControlBackToApplication;

      rc = SQLSetStmtAttrW((SQLHSTMT)tmpHstmt, PY_IFMX_REGISTER_SMART_TRIGGER,
                           (void *)&gSmartTriggerRegister[Counter],
                           SQL_IS_POINTER);
      if (rc == SQL_ERROR) {
        _python_IfxPy_check_sql_errors(conn_res->hdbc, SQL_HANDLE_DBC, rc, 1,
                                       NULL, -1, 1);
      ...
      if (isLoop == TRUE) {
        rc = SQLGetStmtAttr((SQLHSTMT)tmpHstmt,
                            PY_IFMX_GET_DATA_SMART_TRIGGER_LOOP, (void *)&dummy,
                            SQL_NTS, NULL);
      } else {
        rc = SQLGetStmtAttr((SQLHSTMT)tmpHstmt,
                            PY_IFMX_GET_DATA_SMART_TRIGGER_NO_LOOP,
                            (void *)&dummy, SQL_NTS, NULL);
      }
      if (rc == SQL_ERROR) {
        _python_IfxPy_check_sql_errors(conn_res->hdbc, SQL_HANDLE_DBC, rc, 1,
                                       NULL, -1, 1);
...
static PyObject *IfxPy_register_smart_trigger_no_loop(PyObject *self,
                                                      PyObject *args)
{
  return _python_IfxPy_register_smart_trigger_helper(self, args, FALSE);
}

static PyObject *IfxPy_register_smart_trigger_loop(PyObject *self,
                                                   PyObject *args)
{
...
/*    static int _python_IfxPy_assign_options( void *handle, int type, long
 * opt_key, PyObject *data ) */
static int _python_IfxPy_assign_options(void *handle, int type, long opt_key,
                                        PyObject *data)
{
  int rc = 0;
  long option_num = 0;
  ...
  if (opt_key == ATTR_CASE) {
    ...
    if (type == SQL_HANDLE_STMT) {
      ...
      default:
        PyErr_SetString(PyExc_Exception,
                        "ATTR_CASE attribute must be one of CASE_LOWER, "
                        "CASE_UPPER, or CASE_NATURAL");
        return -1;
      }
    ...
    if (type == SQL_HANDLE_STMT) {
      ...
      default:
        PyErr_SetString(
            PyExc_Exception,
            "USE_WCHAR attribute must be one of WCHAR_YES or WCHAR_NO");
        return -1;
      }
    } else if (type == SQL_HANDLE_DBC) {
      switch (option_num) {
    ...
    if (PyString_Check(data) || PyUnicode_Check(data)) {
      ...
      rc = SQLSetStmtAttrW((SQLHSTMT)((stmt_handle *)handle)->hstmt, opt_key,
                           (SQLPOINTER)option_str, SQL_IS_INTEGER);
      if (rc == SQL_ERROR) {
        _python_IfxPy_check_sql_errors((SQLHSTMT)((stmt_handle *)handle)->hstmt,
      ...
      } else if (opt_key == SQL_ATTR_AUTOCOMMIT &&
                 option_num == SQL_AUTOCOMMIT_ON) {
        ((conn_handle *)handle)->auto_commit = 1;
      }

      rc = SQLSetStmtAttr((SQLHSTMT)((stmt_handle *)handle)->hstmt, opt_key,
                          (SQLPOINTER)((SQLLEN)option_num), SQL_IS_INTEGER);
      if (rc == SQL_ERROR) {
        _python_IfxPy_check_sql_errors((SQLHSTMT)((stmt_handle *)handle)->hstmt,
                                       SQL_HANDLE_STMT, rc, 1, NULL, -1, 1);
      }
    ...
    if (PyString_Check(data) || PyUnicode_Check(data)) {
      data = PyUnicode_FromObject(data);
      option_str = getUnicodeDataAsSQLWCHAR(data, &isNewBuffer);
      rc = SQLSetConnectAttrW((SQLHSTMT)((conn_handle *)handle)->hdbc, opt_key,
                              (SQLPOINTER)option_str, SQL_NTS);
      if (rc == SQL_ERROR) {
        _python_IfxPy_check_sql_errors((SQLHSTMT)((stmt_handle *)handle)->hstmt,
                                       SQL_HANDLE_STMT, rc, 1, NULL, -1, 1);
      }
...
/*    static int _python_IfxPy_parse_options( PyObject *options, int type, void
 * *handle)*/
static int _python_IfxPy_parse_options(PyObject *options, int type,
                                       void *handle)
{
...
// initialize the result set information of each column.
...
static int _python_IfxPy_get_result_set_info(stmt_handle *stmt_res)
{
  ...
  if (rc == SQL_ERROR || nResultCols == 0) {
    _python_IfxPy_check_sql_errors((SQLHSTMT)stmt_res->hstmt, SQL_HANDLE_STMT,
                                   rc, 1, NULL, -1, 1);
    return -1;
  }
  ...
  for (i = 0; i < nResultCols; i++) {
    Py_BEGIN_ALLOW_THREADS;
    rc = SQLDescribeCol(
        (SQLHSTMT)stmt_res->hstmt, (SQLSMALLINT)(i + 1), (SQLCHAR *)&tmp_name,
        BUFSIZ, &name_length, &stmt_res->column_info[i].type,
    ...
    if (name_length <= 0) {
      ...
      Py_BEGIN_ALLOW_THREADS;
      rc = SQLDescribeCol(
          (SQLHSTMT)stmt_res->hstmt, (SQLSMALLINT)(i + 1),
...
static int _python_IfxPy_bind_column_helper(stmt_handle *stmt_res)
{
  ...
  for (i = 0; i < stmt_res->num_columns; i++) {
    ...
    case SQL_WVARCHAR:
      ...
      rc =
          SQLBindCol((SQLHSTMT)stmt_res->hstmt, (SQLUSMALLINT)(i + 1),
    ...
    case SQL_TYPE_TIMESTAMP:
      ...
      Py_BEGIN_ALLOW_THREADS;
      rc = SQLBindCol((SQLHSTMT)stmt_res->hstmt, (SQLUSMALLINT)(i + 1),
                      SQL_C_TYPE_TIMESTAMP, row_data->time_val,
...
static PyObject *_python_IfxPy_connect_helper(PyObject *self, PyObject *args,
                                              int isPersistent)
{
  ...
  if (!PyArg_ParseTuple(args, "OOO|OO", &databaseObj, &uidObj, &passwordObj,
                        &options, &literal_replacementObj)) {
    return NULL;
  }
  do {
    ...
    if (isPersistent) {
      ...
      {
        databaseObj =
            PyUnicode_Concat(databaseObj, StringOBJ_FromASCII(";UID="));
        databaseObj = PyUnicode_Concat(databaseObj, uidObj);
        databaseObj =
            PyUnicode_Concat(databaseObj, StringOBJ_FromASCII(";PWD="));
    ...
    if (!conn_res->henv) {
      ...
      if (rc != SQL_SUCCESS) {
        _python_IfxPy_check_sql_errors(conn_res->henv, SQL_HANDLE_ENV, rc, 1,
                                       NULL, -1, 1);
        break;
      }
    ...
    rc = SQLSetConnectAttr((SQLHDBC)conn_res->hdbc, SQL_ATTR_AUTOCOMMIT,
                           (SQLPOINTER)((SQLLEN)(conn_res->auto_commit)),
                           SQL_NTS);
    ...
    if (!NIL_P(options)) {
      if (!PyDict_Check(options)) {
        PyErr_SetString(PyExc_Exception,
                        "options Parameter must be of type dictionay");
        return NULL;
      }
    ...
    if (!reused) {
      ...
      // connect to Informix database
      {
        ...
        rc = SQLDriverConnectW((SQLHDBC)conn_res->hdbc, // ConnectionHandle
                               NULL,                    // WindowHandle
                               ConnStrIn,
        ...
        // if you received an object from Python code and it was a new
        // reference, but you don’t want it to survive past the end of your own
        // C code, you should DECREF it.
      ...
      // LO_AUTO
      rc =
          SQLSetConnectAttr((SQLHDBC)conn_res->hdbc, SQL_INFX_ATTR_LO_AUTOMATIC,
      ...
      Py_BEGIN_ALLOW_THREADS;
      rc = SQLGetInfo(conn_res->hdbc, SQL_DBMS_NAME, (SQLPOINTER)server, 2048,
                      NULL);
      Py_END_ALLOW_THREADS;
    }
...
// This function takes a SQLWCHAR buffer (UCS-2) and returns back a PyUnicode
// object
...
static PyObject *getSQLWCharAsPyUnicodeObject(SQLWCHAR *sqlwcharData,
                                              SQLLEN sqlwcharBytesLen)
{
  ...
  if (maxuniValue <= 65536) {
    ...
    return PyUnicode_FromUnicode((Py_UNICODE *)sqlwcharData,
                                 sqlwcharBytesLen / sizeof(SQLWCHAR));
  }
  ...
  if (is_bigendian()) {
    ...
    if (sizeof(SQLWCHAR) == 4)
      u = PyUnicode_DecodeUTF32((char *)sqlwcharData, sqlwcharBytesLen,
                                "strict", &bo);
    else
      u = PyUnicode_DecodeUTF16((char *)sqlwcharData, sqlwcharBytesLen,
                                "strict", &bo);
  } else {
    int bo = -1;
...
// This function takes value as pyObject and convert it to SQLWCHAR and return
// it
    ...
    if (PyTuple_Size(args) == 2) {
      if (autocommit != (conn_res->auto_commit)) {
        rc = SQLSetConnectAttr(
            (SQLHDBC)conn_res->hdbc, SQL_ATTR_AUTOCOMMIT,
            (SQLPOINTER)((SQLULEN)(autocommit == 0 ? SQL_AUTOCOMMIT_OFF
                                                   : SQL_AUTOCOMMIT_ON)),
            SQL_IS_INTEGER);
        if (rc == SQL_ERROR) {
          _python_IfxPy_check_sql_errors(conn_res->hdbc, SQL_HANDLE_DBC, rc, 1,
...
static void _python_IfxPy_add_param_cache(stmt_handle *stmt_res, int param_no,
                                          PyObject *var_pyvalue, int param_type,
                                          int size, SQLSMALLINT data_type,
                                          SQLULEN precision, SQLSMALLINT scale,
                                          SQLSMALLINT nullable)
{
  param_node *tmp_curr = NULL, *prev = stmt_res->head_cache_list,
...
// static PyObject *_python_IfxPy_bind_param_helper(int argc, stmt_handle
// *stmt_res, SQLUSMALLINT param_no, PyObject *var_pyvalue, long param_type,
//                      long data_type, long precision, long scale, long size)

static PyObject *
_python_IfxPy_bind_param_helper(int argc, stmt_handle *stmt_res,
                                SQLUSMALLINT param_no, PyObject *var_pyvalue,
                                long param_type, long data_type, long precision,
                                long scale, long size)
{
  ...
  case 3:
    ...
    Py_BEGIN_ALLOW_THREADS;
    rc = SQLDescribeParam((SQLHSTMT)stmt_res->hstmt, (SQLUSMALLINT)param_no,
                          &sql_data_type, &sql_precision, &sql_scale,
                          &sql_nullable);
    Py_END_ALLOW_THREADS;
    ...
    if (rc == SQL_ERROR) {
      sprintf(error, "Describe Param Failed: %s", IFX_G(__python_stmt_err_msg));
      PyErr_SetString(PyExc_Exception, error);
      return NULL;
    }
    /* Add to cache */
    _python_IfxPy_add_param_cache(stmt_res, param_no, var_pyvalue, param_type,
                                  size, sql_data_type, sql_precision, sql_scale,
                                  sql_nullable);
    break;
...
static PyObject *IfxPy_bind_param(PyObject *self, PyObject *args)
{
  ...
  if (!PyArg_ParseTuple(args, "OOO|OOOOO", &py_stmt_res, &py_param_no,
                        &var_pyvalue, &py_param_type, &py_data_type,
                        &py_precision, &py_scale, &py_size)) {
    return NULL;
  }
  ...
  if (py_size != NULL && py_size != Py_None && TYPE(py_size) == PYTHON_FIXNUM) {
  ...
  if (!NIL_P(py_stmt_res)) {
    ...
    return _python_IfxPy_bind_param_helper((int)PyTuple_Size(args), stmt_res,
                                           param_no, var_pyvalue, param_type,
                                           data_type, precision, scale, size);
  } else {
    PyErr_SetString(PyExc_Exception, "Supplied parameter is invalid");
    return NULL;
  }
...
static PyObject *IfxPy_close(PyObject *self, PyObject *args)
{
  ...
  conn_handle *conn_res = NULL;
  int rc;
  struct storeSessionID *tempNode = NULL;
  storeSessionID *delNode = NULL;
...
static PyObject *IfxPy_column_privileges(PyObject *self, PyObject *args)
{
  ...
  if (py_table_name != NULL && py_table_name != Py_None) {
    if (PyString_Check(py_table_name) || PyUnicode_Check(py_table_name)) {
      ...
      PyErr_SetString(PyExc_Exception,
                      "table_name must be a string or unicode");
      Py_XDECREF(py_qualifier);
      Py_XDECREF(py_owner);
      return NULL;
    }
...
static PyObject *IfxPy_columns(PyObject *self, PyObject *args)
{
  ...
  if (!NIL_P(py_conn_res)) {
    ...
    Py_BEGIN_ALLOW_THREADS;
    rc = SQLColumnsW((SQLHSTMT)stmt_res->hstmt, qualifier, SQL_NTS, owner,
                     SQL_NTS, table_name, SQL_NTS, column_name, SQL_NTS);
    Py_END_ALLOW_THREADS;
 ...
 *
 * ====pk_schema
 *        The schema for the pk_table-name argument which contains the tables.
 * If schema is NULL, IfxPy.foreign_keys() matches the schema for the current
 * connection.
 *
 * ====pk_table-name
 ...
 *
 * ====fk_schema
 *        The schema for the fk_table-name argument which contains the tables.
 * If schema is NULL, IfxPy.foreign_keys() matches the schema for the current
 * connection.
 *
 * ====fk_table-name
...
static PyObject *IfxPy_foreign_keys(PyObject *self, PyObject *args)
{
  ...
  if (py_pk_qualifier != NULL && py_pk_qualifier != Py_None) {
    if (PyString_Check(py_pk_qualifier) || PyUnicode_Check(py_pk_qualifier)) {
      ...
      PyErr_SetString(PyExc_Exception, "qualifier for table containing primary "
                                       "key must be a string or unicode");
      return NULL;
    }
  }
  ...
  if (py_pk_table_name != NULL && py_pk_table_name != Py_None) {
    if (PyString_Check(py_pk_table_name) || PyUnicode_Check(py_pk_table_name)) {
      ...
      PyErr_SetString(PyExc_Exception,
                      "name of the table that contains primary key must be a "
                      "string or unicode");
      Py_XDECREF(py_pk_qualifier);
  ...
  if (py_fk_qualifier != NULL && py_fk_qualifier != Py_None) {
    if (PyString_Check(py_fk_qualifier) || PyUnicode_Check(py_fk_qualifier)) {
      ...
      PyErr_SetString(PyExc_Exception,
                      "qualifier for table containing the foreign key must be "
                      "a string or unicode");
      Py_XDECREF(py_pk_qualifier);
      Py_XDECREF(py_pk_owner);
  ...
  if (py_fk_owner != NULL && py_fk_owner != Py_None) {
    if (PyString_Check(py_fk_owner) || PyUnicode_Check(py_fk_owner)) {
      ...
      PyErr_SetString(PyExc_Exception, "owner of table containing the foreign "
                                       "key must be a string or unicode");
      Py_XDECREF(py_pk_qualifier);
      Py_XDECREF(py_pk_owner);
      Py_XDECREF(py_pk_table_name);
  ...
  if (py_fk_table_name != NULL && py_fk_table_name != Py_None) {
    if (PyString_Check(py_fk_table_name) || PyUnicode_Check(py_fk_table_name)) {
      ...
      PyErr_SetString(PyExc_Exception,
                      "name of the table that contains foreign key must be a "
                      "string or unicode");
      Py_XDECREF(py_pk_qualifier);
      Py_XDECREF(py_pk_owner);
      Py_XDECREF(py_pk_table_name);
  ...
  if (!NIL_P(py_conn_res)) {
    ...
    Py_BEGIN_ALLOW_THREADS;
    rc =
        SQLForeignKeysW((SQLHSTMT)stmt_res->hstmt, pk_qualifier, SQL_NTS,
                        pk_owner, SQL_NTS, pk_table_name, SQL_NTS, fk_qualifier,
                        SQL_NTS, fk_owner, SQL_NTS, fk_table_name, SQL_NTS);
    Py_END_ALLOW_THREADS;
...
static PyObject *IfxPy_primary_keys(PyObject *self, PyObject *args)
{
  ...
  if (!NIL_P(py_conn_res)) {
    ...
    Py_BEGIN_ALLOW_THREADS;
    rc = SQLPrimaryKeysW((SQLHSTMT)stmt_res->hstmt, qualifier, SQL_NTS, owner,
...
static PyObject *IfxPy_procedure_columns(PyObject *self, PyObject *args)
{
  ...
  if (py_column_name != NULL && py_column_name != Py_None) {
    if (PyString_Check(py_column_name) || PyUnicode_Check(py_column_name)) {
      ...
      PyErr_SetString(PyExc_Exception,
                      "column_name must be a string or unicode");
      Py_XDECREF(py_qualifier);
      Py_XDECREF(py_owner);
...
static PyObject *IfxPy_tables(PyObject *self, PyObject *args)
{
  ...
  if (py_table_type != NULL && py_table_type != Py_None) {
    if (PyString_Check(py_table_type) || PyUnicode_Check(py_table_type)) {
      ...
      PyErr_SetString(PyExc_Exception,
                      "table type must be a string or unicode");
      Py_XDECREF(py_qualifier);
      Py_XDECREF(py_owner);
      Py_XDECREF(py_table_name);
      return NULL;
...
// static int _python_IfxPy_do_prepare(SQLHANDLE hdbc, char *stmt_string,
// stmt_handle *stmt_res, PyObject *options)
static int _python_IfxPy_do_prepare(SQLHANDLE hdbc, SQLWCHAR *stmt,
                                    int stmt_size, stmt_handle *stmt_res,
                                    PyObject *options)
{
  int rc;
  ...
  if (NIL_P(stmt)) {
    PyErr_SetString(PyExc_Exception, "Supplied statement parameter is invalid");
    return rc;
  }
  ...
  Py_BEGIN_ALLOW_THREADS;
  rc = SQLPrepareW((SQLHSTMT)stmt_res->hstmt, stmt, stmt_size);
  Py_END_ALLOW_THREADS;
 ...
 * calling IfxPy.prepare() to prepare an SQL statement with parameter markers
 * * for input values. Then you can call IfxPy.execute() to pass in the input
 ...
 * to fetch rows non-sequentially from the database server. However, it is only
 * supported by DB2 servers, and is much slower than forward-only cursors.
 *
 * ===Return Values
 *
...
static PyObject *IfxPy_exec(PyObject *self, PyObject *args)
{
  ...
  if (!NIL_P(py_conn_res)) {
    ...
    Py_END_ALLOW_THREADS;
    if (rc < SQL_SUCCESS) {
      _python_IfxPy_check_sql_errors(stmt_res->hstmt, SQL_HANDLE_STMT, -1, 1,
...
static PyObject *_python_IfxPy_prepare_helper(conn_handle *conn_res,
                                              PyObject *py_stmt,
                                              PyObject *options)
{
  stmt_handle *stmt_res;
  ...
  if (py_stmt != NULL && py_stmt != Py_None) {
    if (PyString_Check(py_stmt) || PyUnicode_Check(py_stmt)) {
      ...
      if (py_stmt != NULL && py_stmt != Py_None) {
        ...
        PyErr_SetString(PyExc_Exception,
                        "Error occure during processing of statement");
        return NULL;
      }
    } else {
      PyErr_SetString(PyExc_Exception, "statement must be a string or unicode");
  ...
  rc = _python_IfxPy_do_prepare(conn_res->hdbc, stmt, stmt_size, stmt_res,
                                options);
  if (isNewBuffer) {
    if (stmt)
      PyMem_Del(stmt);
...
static param_node *build_list(stmt_handle *stmt_res, int param_no,
                              SQLSMALLINT data_type, SQLULEN precision,
...
static int _python_IfxPy_bind_data(stmt_handle *stmt_res, param_node *curr,
                                   PyObject *bind_data)
{
  int rc;
  ...
  if (curr->param_type == PARAM_FILE) {
    ...
    curr->svalue = memcpy(PyMem_Malloc((sizeof(char)) * (curr->ivalue + 1)),
                          curr->svalue, curr->ivalue);
    ...
    Py_BEGIN_ALLOW_THREADS;
    SQLINTEGER temp_indicator = (SQLINTEGER)curr->bind_indicator;
    rc = SQLBindFileToParam(
        (SQLHSTMT)stmt_res->hstmt, curr->param_num, curr->data_type,
        (SQLCHAR *)curr->svalue, (SQLSMALLINT *)&(curr->ivalue),
        &(curr->file_options), curr->ivalue, &temp_indicator);
    Py_END_ALLOW_THREADS;
  ...
  case PYTHON_FIXNUM:
    if (curr->data_type == SQL_BIGINT || curr->data_type == SQL_DECIMAL) {
      ...
      Py_BEGIN_ALLOW_THREADS;
      rc = SQLBindParameter(stmt_res->hstmt, curr->param_num, curr->param_type,
                            SQL_C_CHAR, curr->data_type, curr->param_size,
                            curr->scale, curr->svalue, curr->param_size, NULL);
      Py_END_ALLOW_THREADS;
      ...
      Py_BEGIN_ALLOW_THREADS;
      rc = SQLBindParameter(stmt_res->hstmt, curr->param_num, curr->param_type,
                            SQL_C_LONG, curr->data_type, curr->param_size,
  ...
  case PYTHON_FLOAT:
    ...
    Py_BEGIN_ALLOW_THREADS;
    rc = SQLBindParameter(stmt_res->hstmt, curr->param_num, curr->param_type,
                          SQL_C_DOUBLE, curr->data_type, curr->param_size,
  ...
  case PYTHON_UNICODE: {
    ...
    if (PyObject_CheckBuffer(bind_data) &&
        (curr->data_type == SQL_BINARY || curr->data_type == SQL_VARBINARY ||
         ...
         curr->data_type == SQL_INFX_RC_SET)) {
#if PY_MAJOR_VERSION >= 3
...
#else
      PyObject_AsReadBuffer(bind_data, (const void **)&(curr->uvalue),
                            &buffer_len);
    ...
    Py_BEGIN_ALLOW_THREADS;
    rc = SQLBindParameter(stmt_res->hstmt, curr->param_num, curr->param_type,
                          valueType, curr->data_type, curr->param_size,
                          curr->scale, paramValuePtr, curr->ivalue,
                          &(curr->bind_indicator));
  ...
  case PYTHON_STRING: {
    ...
    if (PyObject_CheckBuffer(bind_data) &&
      ...
      curr->ivalue = tmp_buffer.len;
#else
      PyObject_AsReadBuffer(bind_data, (const void **)&(curr->svalue),
                            &buffer_len);
      curr->ivalue = buffer_len;
#endif
    } else {
      ...
      curr->svalue = PyBytes_AsString(
          bind_data); // It is PyString_AsString() in PY_MAJOR_VERSION<3, and
                      // code execution will not come here in
                      // PY_MAJOR_VERSION>=3
      curr->ivalue = strlen(curr->svalue);
    }
    param_length = curr->ivalue;
  ...
  case PYTHON_DATE:
    ...
    Py_BEGIN_ALLOW_THREADS;
    rc = SQLBindParameter(stmt_res->hstmt, curr->param_num, curr->param_type,
                          SQL_C_TYPE_DATE, curr->data_type, curr->param_size,
                          curr->scale, curr->date_value, curr->ivalue,
                          &(curr->bind_indicator));
    Py_END_ALLOW_THREADS;
    break;
  ...
  case PYTHON_TIME:
    ...
    Py_BEGIN_ALLOW_THREADS;
    rc = SQLBindParameter(stmt_res->hstmt, curr->param_num, curr->param_type,
                          SQL_C_TYPE_TIME, curr->data_type, curr->param_size,
                          curr->scale, curr->time_value, curr->ivalue,
                          &(curr->bind_indicator));
    Py_END_ALLOW_THREADS;
    break;
  ...
  case PYTHON_TIMESTAMP:
    ...
    curr->ts_value->fraction =
        PyDateTime_DATE_GET_MICROSECOND(bind_data) * 1000;

    Py_BEGIN_ALLOW_THREADS;
    rc = SQLBindParameter(stmt_res->hstmt, curr->param_num, curr->param_type,
                          SQL_C_TYPE_TIMESTAMP, curr->data_type,
                          curr->param_size, curr->scale, curr->ts_value,
  ...
  case PYTHON_TIMEDELTA:
    ...
    if (((PyDateTime_Delta *)bind_data)->days < 0) {
      ...
      curr->interval_value->intval.day_second.day =
          (((PyDateTime_Delta *)bind_data)->days * -1) - 1;
      curr->interval_value->intval.day_second.second =
          86400 - (((PyDateTime_Delta *)bind_data)->seconds);
    } else {
      curr->interval_value->interval_sign = 1;
      curr->interval_value->intval.day_second.day =
          ((PyDateTime_Delta *)bind_data)->days;
      curr->interval_value->intval.day_second.second =
          (((PyDateTime_Delta *)bind_data)->seconds);
    }
    curr->interval_value->intval.day_second.hour =
        curr->interval_value->intval.day_second.second / 3600;
    curr->interval_value->intval.day_second.second -=
        curr->interval_value->intval.day_second.hour * 3600;
    curr->interval_value->intval.day_second.minute =
        curr->interval_value->intval.day_second.second / 60;
    curr->interval_value->intval.day_second.second -=
        curr->interval_value->intval.day_second.minute * 60;
    curr->interval_value->intval.day_second.fraction = 0;

    Py_BEGIN_ALLOW_THREADS;
    rc = SQLBindParameter(stmt_res->hstmt, curr->param_num, curr->param_type,
                          SQL_C_INTERVAL_DAY_TO_SECOND, curr->data_type,
                          curr->param_size, curr->scale, curr->interval_value,
                          curr->ivalue, &(curr->bind_indicator));
    Py_END_ALLOW_THREADS;
    break;
  ...
  case PYTHON_NIL:
    ...
    Py_BEGIN_ALLOW_THREADS;
    rc = SQLBindParameter(stmt_res->hstmt, curr->param_num, curr->param_type,
                          SQL_C_DEFAULT, curr->data_type, curr->param_size,
                          curr->scale, &curr->ivalue, 0,
                          (SQLLEN *)&(curr->ivalue));
    Py_END_ALLOW_THREADS;
...
static int _python_IfxPy_execute_helper2(stmt_handle *stmt_res, PyObject *data,
                                         int bind_cmp_list, int bind_params)
{
  int rc = SQL_SUCCESS;
  ...
  if (bind_cmp_list) {
    ...
    while (curr != NULL) {
      ...
      if (rc == SQL_ERROR) {
        sprintf(error, "Binding Error 1: %s", IFX_G(__python_stmt_err_msg));
        PyErr_SetString(PyExc_Exception, error);
        return rc;
      }
      curr = curr->next;
    ...
    if (data != NULL) {
      if (bind_params) {
        ...
        rc = SQLDescribeParam((SQLHSTMT)stmt_res->hstmt, param_no,
                              (SQLSMALLINT *)&data_type, &precision,
                              (SQLSMALLINT *)&scale, (SQLSMALLINT *)&nullable);
        Py_END_ALLOW_THREADS;
        ...
        curr = build_list(stmt_res, param_no, data_type, precision, scale,
                          nullable);
        rc = _python_IfxPy_bind_data(stmt_res, curr, data);
        if (rc == SQL_ERROR) {
          sprintf(error, "Binding Error 2: %s", IFX_G(__python_stmt_err_msg));
          PyErr_SetString(PyExc_Exception, error);
          return rc;
        }
      } else {
...
static PyObject *_python_IfxPy_execute_helper1(stmt_handle *stmt_res,
                                               PyObject *parameters_tuple)
{
  ...
  if (num != 0) {
    ...
    if (!NIL_P(parameters_tuple)) {
      ...
      if (numOpts > num) {
        ...
        // The z portion is a length specifier which says the argument will be
        // size_t in length
        // https://en.wikipedia.org/wiki/Printf_format_string#printf_format_placeholders
        sprintf(error, "%zu params bound not matching %d required", numOpts,
                num);
        PyErr_SetString(PyExc_Exception, error);
  ...
  if (rc == SQL_NEED_DATA) {
    ...
    while (rc == SQL_NEED_DATA) {
      ...
      if (!NIL_P(((param_node *)valuePtr)->svalue)) {
        ...
        Py_BEGIN_ALLOW_THREADS;
        rc = SQLPutData((SQLHSTMT)stmt_res->hstmt,
                        (SQLPOINTER)(((param_node *)valuePtr)->uvalue),
                        ((param_node *)valuePtr)->ivalue);
        Py_END_ALLOW_THREADS;
      }
 ...
 * ===Return Values
 *
 * Returns a string containing the warning message and SQLCODE value for the
 * last warning that occurred issuing an SQL statement.
 */
...
static int _python_IfxPy_get_column_by_name(stmt_handle *stmt_res,
...
static PyObject *IfxPy_field_name(PyObject *self, PyObject *args)
{
  ...
  if (NIL_P(py_stmt_res) ||
      (!PyObject_TypeCheck(py_stmt_res, &stmt_handleType))) {
    PyErr_SetString(PyExc_Exception,
                    "Supplied statement object parameter is invalid");
    return NULL;
  } else {
...
static PyObject *IfxPy_field_display_size(PyObject *self, PyObject *args)
{
  ...
  // In ODBC 3.x, the ODBC 2.0 function SQLColAttributes has been replaced by
  // SQLColAttribute
  ...
  rc = SQLColAttribute((SQLHSTMT)stmt_res->hstmt, (SQLSMALLINT)col + 1,
                       SQL_DESC_DISPLAY_SIZE, NULL, 0, NULL,
                       &colDataDisplaySize);
  Py_END_ALLOW_THREADS;
 ...
 *
 * Returns True/False based on indicated column in result set is nullable or
 * not.
 *
 * ===Parameters
 *
 ...
 * integer representing the 0-indexed position of the column, or a string
 ...
 * IfxPy.field_nullable() returns FALSE
 */
  ...
  if (!NIL_P(py_stmt_res)) {
    if (PyObject_TypeCheck(py_stmt_res, &stmt_handleType)) {
      ...
      if (handle->hstmt != NULL) {
        ...
        if (rc == SQL_ERROR || rc == SQL_SUCCESS_WITH_INFO) {
          _python_IfxPy_check_sql_errors(handle->hstmt, SQL_HANDLE_STMT, rc, 1,
                                         NULL, -1, 1);
        }
        if (rc == SQL_ERROR) {
          Py_RETURN_FALSE;
...
static RETCODE _python_IfxPy_get_data(stmt_handle *stmt_res, int col_num,
                                      short ctype, void *buff, SQLLEN in_length,
                                      SQLLEN *out_length)
{
 ...
 *
 * Use IfxPy.result() to return the value of a specified column in the current
 * * row of a result set. You must call IfxPy.fetch_row() before calling
...
static PyObject *IfxPy_result(PyObject *self, PyObject *args)
{
  ...
  if (!NIL_P(py_stmt_res)) {
    ...
    if (TYPE(column) == PYTHON_FIXNUM) {
      ...
      column = col_name_py3_tmp;
#endif
      col_num = _python_IfxPy_get_column_by_name(stmt_res,
                                                 PyBytes_AsString(column), -1);
#if PY_MAJOR_VERSION >= 3
      if (col_name_py3_tmp != NULL) {
        Py_XDECREF(col_name_py3_tmp);
      }
    ...
    case SQL_INFX_UDT_VARYING:
      ...
      rc = _python_IfxPy_get_data(stmt_res, col_num + 1, SQL_C_WCHAR, out_ptr,
                                  in_length * sizeof(Py_UNICODE), &out_length);
    ...
    case SQL_TYPE_DATE:
      ...
      if (out_length == SQL_NULL_DATA) {
        ...
        return_value =
            PyDate_FromDate(date_ptr->year, date_ptr->month, date_ptr->day);
        PyMem_Del(date_ptr);
        date_ptr = NULL;
        return return_value;
      }
    ...
    case SQL_TYPE_TIME:
      ...
      if (out_length == SQL_NULL_DATA) {
        ...
        return_value = PyTime_FromTime(time_ptr->hour, time_ptr->minute,
                                       time_ptr->second, 0);
        PyMem_Del(time_ptr);
        time_ptr = NULL;
        return return_value;
    ...
    case SQL_TYPE_TIMESTAMP:
      ...
      rc =
          _python_IfxPy_get_data(stmt_res, col_num + 1, SQL_C_TYPE_TIMESTAMP,
      ...
      if (out_length == SQL_NULL_DATA) {
        ...
        return_value = PyDateTime_FromDateAndTime(
            ts_ptr->year, ts_ptr->month, ts_ptr->day, ts_ptr->hour,
            ts_ptr->minute, ts_ptr->second, ts_ptr->fraction / 1000);
        PyMem_Del(ts_ptr);
        ts_ptr = NULL;
        return return_value;
      }
    ...
    case SQL_INTERVAL_MINUTE_TO_SECOND:
      ...
      rc = _python_IfxPy_get_data(stmt_res, col_num + 1, column_type,
                                  interval_ptr, sizeof(SQL_INTERVAL_STRUCT),
                                  &out_length);
      ...
      if (out_length == SQL_NULL_DATA) {
        ...
        if (interval_ptr->interval_sign == 0) {
          return_value = PyDelta_FromDSU(
              (interval_ptr->intval.day_second.day * -1) - 1,
              (86400 - (interval_ptr->intval.day_second.hour * 3600) +
               (interval_ptr->intval.day_second.minute * 60) +
               interval_ptr->intval.day_second.second),
              0);
        } else {
          return_value =
              PyDelta_FromDSU(interval_ptr->intval.day_second.day,
    ...
    case SQL_SMALLINT:
    case SQL_INTEGER:
      rc = _python_IfxPy_get_data(stmt_res, col_num + 1, SQL_C_LONG, &long_val,
                                  sizeof(long_val), &out_length);
      if (rc == SQL_ERROR) {
        PyErr_Clear();
        Py_RETURN_FALSE;
      }
    ...
    case SQL_DOUBLE:
      rc = _python_IfxPy_get_data(stmt_res, col_num + 1, SQL_C_DOUBLE,
                                  &double_val, sizeof(double_val), &out_length);
      if (rc == SQL_ERROR) {
        PyErr_Clear();
        Py_RETURN_FALSE;
      }
    ...
    case SQL_VARBINARY:
      ...
      if (rc == SQL_SUCCESS_WITH_INFO) {
        ...
        rc = _python_IfxPy_get_data(stmt_res, col_num + 1, targetCType,
        ...
        if (len_terChar == sizeof(SQLWCHAR)) {
          retVal =
              getSQLWCharAsPyUnicodeObject(out_ptr, INIT_BUFSIZ + out_length);
        } else {
...
static PyObject *_python_IfxPy_bind_fetch_helper(PyObject *args, int op)
{
  ...
  for (column_number = 0; column_number < stmt_res->num_columns;
       column_number++) {
    ...
    case CASE_LOWER:
      stmt_res->column_info[column_number].name = (SQLCHAR *)strtolower(
          (char *)stmt_res->column_info[column_number].name,
          (int)strlen((char *)stmt_res->column_info[column_number].name));
      break;

    case CASE_UPPER:
      stmt_res->column_info[column_number].name = (SQLCHAR *)strtoupper(
          (char *)stmt_res->column_info[column_number].name,
          (int)strlen((char *)stmt_res->column_info[column_number].name));
      break;
    ...
    if (out_length == SQL_NULL_DATA) {
      ...
      case SQL_VARCHAR:
        if (stmt_res->s_use_wchar == WCHAR_NO) {
          ...
          value =
              PyBytes_FromStringAndSize((char *)row_data->str_val, out_length);
          break;
        }
      case SQL_WCHAR:
      case SQL_WVARCHAR:
      ...
      case SQL_WLONGVARCHAR:
        ...
        rc = _python_IfxPy_get_data(
            stmt_res, column_number + 1, SQL_C_WCHAR, wout_ptr,
      ...
      case SQL_TYPE_DATE:
        value =
            PyDate_FromDate(row_data->date_val->year, row_data->date_val->month,
                            row_data->date_val->day);
        break;

      case SQL_TYPE_TIME:
        value = PyTime_FromTime(row_data->time_val->hour,
                                row_data->time_val->minute,
                                row_data->time_val->second, 0);
        break;

      case SQL_TYPE_TIMESTAMP:
        value = PyDateTime_FromDateAndTime(
            row_data->ts_val->year, row_data->ts_val->month,
            row_data->ts_val->day, row_data->ts_val->hour,
            row_data->ts_val->minute, row_data->ts_val->second,
      ...
      case SQL_INTERVAL_MINUTE_TO_SECOND:
        if (row_data->interval_val->interval_sign == 0) {
          value = PyDelta_FromDSU(
              (row_data->interval_val->intval.day_second.day * -1) - 1,
              (86400 - (row_data->interval_val->intval.day_second.hour * 3600) +
               (row_data->interval_val->intval.day_second.minute * 60) +
               row_data->interval_val->intval.day_second.second),
              0);
        } else {
          value = PyDelta_FromDSU(
              row_data->interval_val->intval.day_second.day,
      ...
      case SQL_VARBINARY:
        if (stmt_res->s_bin_mode == PASSTHRU) {
          ...
          if (stmt_res->column_info[column_number].size < INT_MAX) {
            value = PyBytes_FromStringAndSize((char *)row_data->str_val,
                                              out_length);
          } else {
            tmp_length = out_length;
            ...
            rc = _python_IfxPy_get_data(
                stmt_res, column_number + 1, SQL_C_BINARY, wout_ptr,
            ...
            if (out_length == SQL_NULL_DATA) {
              ...
              char *temp_str = (char *)malloc(out_length + 1);
              for (int i = 0; i < out_length; i++) {
                temp_str[i] = (char)wout_ptr[i];
              }
              temp_str[out_length] = '\0';
              value = PyBytes_FromStringAndSize(temp_str, out_length);
              free(temp_str);
            }
            if (wout_ptr != NULL) {
              PyMem_Del(wout_ptr);
      ...
      case SQL_INFX_UDT_FIXED:
      case SQL_INFX_UDT_VARYING:
        if (column_type == SQL_INFX_RC_SET ||
            column_type == SQL_INFX_RC_MULTISET ||
            column_type == SQL_INFX_RC_COLLECTION ||
            column_type == SQL_INFX_RC_ROW ||
            column_type == SQL_INFX_UDT_FIXED ||
            column_type == SQL_INFX_UDT_VARYING) {
        ...
        rc = _python_IfxPy_get_data(stmt_res, column_number + 1, targetCType,
                                    out_ptr, INIT_BUFSIZ + len_terChar,
                                    &out_length);
        if (rc == SQL_SUCCESS_WITH_INFO) {
          void *tmp_out_ptr = NULL;

          tmp_out_ptr =
              (void *)ALLOC_N(char, out_length + INIT_BUFSIZ + len_terChar);
          memcpy(tmp_out_ptr, out_ptr, INIT_BUFSIZ);
          PyMem_Del(out_ptr);
          out_ptr = tmp_out_ptr;
 ...
 *
 * Returns a dictionary, indexed by column name, representing a row in a result
 * * set.
 *
 * ===Parameters
 * ====stmt
...
/*
 * IfxPy.fetch_object --    Returns an object with properties representing
 ...
 * row_number
 *        Requests a specific 1-indexed row from the result set. Passing this
 * parameter results in a Python warning if the result set uses a forward-only
 * cursor.
 *
 * ===Return Values
 *
 ...
 *
 * Returns a dictionary, indexed by both column name and position, representing
 * * a row in a result set. Note that the row returned by IfxPy.fetch_both()
 * requires more memory than the single-indexed dictionaries/arrays returned by
 * * IfxPy.fetch_assoc() or IfxPy.fetch_tuple().
 *
 * ===Parameters
 *
 * ====stmt
...
static PyObject *IfxPy_server_info(PyObject *self, PyObject *args)
{
  ...
  le_server_info *return_value = PyObject_NEW(le_server_info, &server_infoType);
...
*
* This function returns a read-only object with information about the IDS
client. The following table lists the client properties:
*
...
static PyObject *IfxPy_client_info(PyObject *self, PyObject *args)
{
  ...
  le_client_info *return_value = PyObject_NEW(le_client_info, &client_infoType);
  ...
  if (!NIL_P(py_conn_res)) {
    ...
    Py_BEGIN_ALLOW_THREADS;
    rc = SQLGetInfo(conn_res->hdbc, SQL_DATA_SOURCE_NAME, (SQLPOINTER)buffer255,
                    sizeof(buffer255), NULL);
    Py_END_ALLOW_THREADS;
    ...
    Py_BEGIN_ALLOW_THREADS;
    rc = SQLGetInfo(conn_res->hdbc, SQL_DRIVER_ODBC_VER, (SQLPOINTER)buffer255,
                    sizeof(buffer255), NULL);
    Py_END_ALLOW_THREADS;
    ...
    if (type == 1) {
      ...
      if (!NIL_P(py_op_integer)) {
        ...
        if (op_integer == SQL_ATTR_CURSOR_TYPE) {
          rc = SQLGetStmtAttr((SQLHSTMT)stmt_res->hstmt, op_integer, &value_int,
                              SQL_IS_INTEGER, NULL);
          if (rc == SQL_ERROR) {
            _python_IfxPy_check_sql_errors(stmt_res->hstmt, SQL_HANDLE_STMT, rc,
                                           1, NULL, -1, 1);
...
static void _build_client_err_list(error_msg_node *head_error_list,
                                   char *err_msg)
{
  error_msg_node *tmp_err = NULL, *curr_err = head_error_list->next,
                 *prv_err = NULL;
...
// IfxPy.execute_many -- can be used to execute an SQL with multiple values of
// parameter marker.
//===Description
// int IfxPy.execute_many(IFXStatement, Parameters[, Options])
// Returns number of inserted/updated/deleted rows if batch executed
// successfully. return NULL if batch fully or partialy fails  (All the rows
// executed except for which error occurs).
...
static PyObject *IfxPy_execute_many(PyObject *self, PyObject *args)
{
  ...
  if (!NIL_P(py_stmt_res)) {
    ...
    if (numOpts != 0) {
      SQLULEN temp_precision;
      for (i = 0; i < numOpts; i++) {
        Py_BEGIN_ALLOW_THREADS;
        rc = SQLDescribeParam((SQLHSTMT)stmt_res->hstmt, i + 1,
                              (SQLSMALLINT *)(data_type + i), &temp_precision,
                              (SQLSMALLINT *)&scale, (SQLSMALLINT *)&nullable);
        Py_END_ALLOW_THREADS;
        precision = (SQLUINTEGER)temp_precision;
        ...
        build_list(stmt_res, i + 1, data_type[i], precision, scale, nullable);
      }
    }
    ...
    if (numOfRows > 0) {
      for (i = 0; i < numOfRows; i++) {
        ...
        if (numOpts < numOfParam) {
          // More are passed in -- Warning - Use the max number present
          sprintf(error, "Value parameter tuple: %d has more no of param",
                  i + 1);
          _build_client_err_list(head_error_list, error);
          err_count++;
          continue;
          ...
          sprintf(error, "Value parameter tuple: %d has less no of param",
                  i + 1);
          _build_client_err_list(head_error_list, error);
          err_count++;
          continue;
        ...
        while (curr != NULL) {
          ...
          // if (chaining_start)
          //{
          //     if ((TYPE(data) != PYTHON_NIL) && (ref_data_type
          //     [curr->param_num - 1] != TYPE(data)))
          //     {
          //         sprintf(error, "Value parameters array %d is not
          //         homogeneous with privious parameters array", i + 1);
          //         _build_client_err_list(head_error_list, error);
          //         err_count++;
          //         break;
          //     }
          ...
          if (TYPE(data) != PYTHON_NIL) {
            ...
            rc = SQLBindParameter(stmt_res->hstmt, curr->param_num,
                                  ...
                                  curr->param_size, curr->scale, &curr->ivalue,
                                  0, (SQLLEN *)&(curr->ivalue));
            Py_END_ALLOW_THREADS;
            if (rc == SQL_ERROR || rc == SQL_SUCCESS_WITH_INFO) {
        ...
        //     rc = _IfxPy_chaining_flag(stmt_res, SQL_ATTR_CHAINING_BEGIN,
        //     NULL, 0); chaining_start = 1; if (rc != SQL_SUCCESS)
        //     {
        //         return NULL;
        //     }
        // }
        ...
        if (error[0] == '\0') {
          ...
          if (rc == SQL_NEED_DATA) {
            SQLPOINTER valuePtr;
            rc = SQLParamData((SQLHSTMT)stmt_res->hstmt,
                              (SQLPOINTER *)&valuePtr);
            while (rc == SQL_NEED_DATA) {
              // passing data value for a parameter
              if (!NIL_P(((param_node *)valuePtr)->svalue)) {
    ...
    // rc = _IfxPy_chaining_flag(stmt_res, SQL_ATTR_CHAINING_END,
    // head_error_list->next, err_count); if (head_error_list != NULL)
    //{
  ...
  Py_BEGIN_ALLOW_THREADS;
  SQLLEN temp_row_cnt;
  rc = SQLRowCount((SQLHSTMT)stmt_res->hstmt, &temp_row_cnt);
  row_cnt = (SQLINTEGER)temp_row_cnt;
  Py_END_ALLOW_THREADS;
...
/*
 * ===Description
 *  IfxPy.callproc( conn_handle conn_res, char *procName, (In/INOUT/OUT
 * parameters tuple) )
 *
...
static PyObject *IfxPy_callproc(PyObject *self, PyObject *args)
{
  ...
  if (!PyArg_ParseTuple(args, "OO|O", &py_conn_res, &pyprocName,
                        &parameters_tuple)) {
    return NULL;
  }
  ...
  if (!NIL_P(py_conn_res) && pyprocName != Py_None) {
    ...
    if (!NIL_P(parameters_tuple)) {
      ...
      Py_XDECREF(subsql1);
      strsubsql = (char *)PyMem_Malloc(
          sizeof(char) * ((strlen("(  )") + strlen(", ?") * numOfParam) + 2));
      if (strsubsql == NULL) {
        PyErr_SetString(PyExc_Exception, "Failed to Allocate Memory");
        return NULL;
      }
      ...
      Py_XDECREF(subsql2);
      stmt_res =
          (stmt_handle *)_python_IfxPy_prepare_helper(conn_res, sql, NULL);
      PyMem_Del(strsubsql);
      Py_XDECREF(sql);
      if (NIL_P(stmt_res)) {
        return NULL;
      ...
      for (i = 0; i < numOfParam; i++) {
        ...
        bind_result = _python_IfxPy_bind_param_helper(
            4, stmt_res, i + 1, data, SQL_PARAM_INPUT_OUTPUT, 0, 0, 0, 0);
          ...
          if ((tmp_curr->bind_indicator != SQL_NULL_DATA &&
              ...
              if (!NIL_P(tmp_curr->ts_value)) {
                PyTuple_SetItem(
                    ...
                    PyDateTime_FromDateAndTime(
                        tmp_curr->ts_value->year, tmp_curr->ts_value->month,
                        tmp_curr->ts_value->day, tmp_curr->ts_value->hour,
                        tmp_curr->ts_value->minute, tmp_curr->ts_value->second,
            ...
            case SQL_BIGINT:
              if (!NIL_P(tmp_curr->svalue)) {
                PyTuple_SetItem(outTuple, paramCount,
                                PyLong_FromString(tmp_curr->svalue, NULL, 0));
              } else {
                Py_INCREF(Py_None);
                PyTuple_SetItem(outTuple, paramCount, Py_None);
              }
            ...
            default:
              if (!NIL_P(tmp_curr->svalue)) {
                PyTuple_SetItem(outTuple, paramCount,
                                StringOBJ_FromASCII(tmp_curr->svalue));
                paramCount++;
              } else if (!NIL_P(tmp_curr->uvalue)) {
                PyTuple_SetItem(
                    outTuple, paramCount,
                    getSQLWCharAsPyUnicodeObject(tmp_curr->uvalue,
                                                 tmp_curr->bind_indicator));
                paramCount++;
              } else {
                Py_INCREF(Py_None);
                PyTuple_SetItem(outTuple, paramCount, Py_None);
    ...
    PyErr_SetString(PyExc_Exception,
                    "Connection Resource invalid or procedure name is NULL");
    return NULL;
  }
}

/*
 * IfxPy.check_function_support-- can be used to query whether a  ODBC function
 * is supported
 * ===Description
...
static PyObject *IfxPy_check_function_support(PyObject *self, PyObject *args)
{
  ...
  if (!NIL_P(py_conn_res)) {
    ...
    Py_BEGIN_ALLOW_THREADS;
    rc = SQLGetFunctions(conn_res->hdbc, (SQLUSMALLINT)funtion_id,
...
/*
 * IfxPy.get_last_serial_value --    Gets the last inserted serial value from
 * IDS
 *
 * ===Description
 ...
 * IDS. The last inserted value could be auto-generated or entered explicitly by
 * the user This function is valid for IDS (Informix Dynamic Server only)
 *
 * ===Parameters
 *
 * stmt
 ...
 * ===Return Values
 *
 * Returns a string representation of last inserted serial value on a successful
...
PyObject *IfxPy_get_last_serial_value(int argc, PyObject *args, PyObject *self)
{
  ...
  if (!NIL_P(py_qualifier)) {
    ...
    /* We allocate a buffer of size 31 as per recommendations from the CLI IDS
     * team */
    ...
    rc = SQLGetStmtAttr((SQLHSTMT)stmt_res->hstmt, SQL_ATTR_GET_GENERATED_VALUE,
                        (SQLPOINTER)value, 31, &pcbValue);
    if (rc == SQL_ERROR) {
      _python_IfxPy_check_sql_errors((SQLHSTMT)stmt_res->hstmt, SQL_HANDLE_STMT,
                                     rc, 1, NULL, -1, 1);
      if (value != NULL) {
...
static PyMethodDef IfxPy_Methods[] = {
    ...
    {"connect", (PyCFunction)IfxPy_connect, METH_VARARGS | METH_KEYWORDS,
     "Connect to the database"},
    {"exec_immediate", (PyCFunction)IfxPy_exec, METH_VARARGS,
     "Prepares and executes an SQL statement."},
    {"prepare", (PyCFunction)IfxPy_prepare, METH_VARARGS,
     "Prepares an SQL statement."},
    {"bind_param", (PyCFunction)IfxPy_bind_param, METH_VARARGS,
     "Binds a Python variable to an SQL statement parameter"},
    {"execute", (PyCFunction)IfxPy_execute, METH_VARARGS,
     "Executes an SQL statement that was prepared by IfxPy.prepare()"},
    {"fetch_tuple", (PyCFunction)IfxPy_fetch_array, METH_VARARGS,
     "Returns an tuple, indexed by column position, representing a row in a "
     "result set"},
    {"fetch_assoc", (PyCFunction)IfxPy_fetch_assoc, METH_VARARGS,
     "Returns a dictionary, indexed by column name, representing a row in a "
     "result set"},
    {"fetch_both", (PyCFunction)IfxPy_fetch_both, METH_VARARGS,
     "Returns a dictionary, indexed by both column name and position, "
     "representing a row in a result set"},
    {"fetch_row", (PyCFunction)IfxPy_fetch_row, METH_VARARGS,
     "Sets the result set pointer to the next row or requested row"},
    {"result", (PyCFunction)IfxPy_result, METH_VARARGS,
     "Returns a single column from a row in the result set"},
    {"active", (PyCFunction)IfxPy_active, METH_VARARGS,
     "Checks if the specified connection resource is active"},
    {"autocommit", (PyCFunction)IfxPy_autocommit, METH_VARARGS,
     "Returns or sets the AUTOCOMMIT state for a database connection"},
    {"callproc", (PyCFunction)IfxPy_callproc, METH_VARARGS,
     "Returns a tuple containing OUT/INOUT variable value"},
    {"check_function_support", (PyCFunction)IfxPy_check_function_support,
     METH_VARARGS,
     "return true if fuction is supported otherwise return false"},
    {"close", (PyCFunction)IfxPy_close, METH_VARARGS,
     "Close a database connection"},
    {"conn_error", (PyCFunction)IfxPy_conn_error, METH_VARARGS,
     "Returns a string containing the SQLSTATE returned by the last connection "
     "attempt"},
    {"conn_errormsg", (PyCFunction)IfxPy_conn_errormsg, METH_VARARGS,
     "Returns an error message and SQLCODE value representing the reason the "
     "last database connection attempt failed"},
    {"conn_warn", (PyCFunction)IfxPy_conn_warn, METH_VARARGS,
     "Returns a warning string containing the SQLSTATE returned by the last "
     "connection attempt"},
    {"client_info", (PyCFunction)IfxPy_client_info, METH_VARARGS,
     "Returns a read-only object with information about the IDS database "
     "client"},
    {"column_privileges", (PyCFunction)IfxPy_column_privileges, METH_VARARGS,
     "Returns a result set listing the columns and associated privileges for a "
     "table."},
    {"columns", (PyCFunction)IfxPy_columns, METH_VARARGS,
     "Returns a result set listing the columns and associated metadata for a "
     "table"},
    {"commit", (PyCFunction)IfxPy_commit, METH_VARARGS,
     "Commits a transaction"},
    {"cursor_type", (PyCFunction)IfxPy_cursor_type, METH_VARARGS,
     "Returns the cursor type used by a statement resource"},
    {"execute_many", (PyCFunction)IfxPy_execute_many, METH_VARARGS,
     "Execute SQL with multiple rows."}, // TODO
    {"field_display_size", (PyCFunction)IfxPy_field_display_size, METH_VARARGS,
     "Returns the maximum number of bytes required to display a column"},
    {"field_name", (PyCFunction)IfxPy_field_name, METH_VARARGS,
     "Returns the name of the column in the result set"},
    {"field_nullable", (PyCFunction)IfxPy_field_nullable, METH_VARARGS,
     "Returns indicated column can contain nulls or not"},
    {"field_num", (PyCFunction)IfxPy_field_num, METH_VARARGS,
     "Returns the position of the named column in a result set"},
    {"field_precision", (PyCFunction)IfxPy_field_precision, METH_VARARGS,
     "Returns the precision of the indicated column in a result set"},
    {"field_scale", (PyCFunction)IfxPy_field_scale, METH_VARARGS,
     "Returns the scale of the indicated column in a result set"},
    {"field_type", (PyCFunction)IfxPy_field_type, METH_VARARGS,
     "Returns the data type of the indicated column in a result set"},
    {"field_width", (PyCFunction)IfxPy_field_width, METH_VARARGS,
     "Returns the width of the indicated column in a result set"},
    {"foreign_keys", (PyCFunction)IfxPy_foreign_keys, METH_VARARGS,
     "Returns a result set listing the foreign keys for a table"},
    {"free_result", (PyCFunction)IfxPy_free_result, METH_VARARGS,
     "Frees resources associated with a result set"},
    {"free_stmt", (PyCFunction)IfxPy_free_stmt, METH_VARARGS,
     "Frees resources associated with the indicated statement resource"},
    {"get_option", (PyCFunction)IfxPy_get_option, METH_VARARGS,
     "Gets the specified option in the resource."},
    {"num_fields", (PyCFunction)IfxPy_num_fields, METH_VARARGS,
     "Returns the number of fields contained in a result set"},
    {"num_rows", (PyCFunction)IfxPy_num_rows, METH_VARARGS,
     "Returns the number of rows affected by an SQL statement"},
    {"get_num_result", (PyCFunction)IfxPy_get_num_result, METH_VARARGS,
     "Returns the number of rows in a current open non-dynamic scrollable "
     "cursor"},
    {"primary_keys", (PyCFunction)IfxPy_primary_keys, METH_VARARGS,
     "Returns a result set listing primary keys for a table"},
    {"procedure_columns", (PyCFunction)IfxPy_procedure_columns, METH_VARARGS,
     "Returns a result set listing the parameters for one or more stored "
     "procedures."},
    {"procedures", (PyCFunction)IfxPy_procedures, METH_VARARGS,
     "Returns a result set listing the stored procedures registered in a "
     "database"},
    {"rollback", (PyCFunction)IfxPy_rollback, METH_VARARGS,
     "Rolls back a transaction"},
    {"server_info", (PyCFunction)IfxPy_server_info, METH_VARARGS,
     "Returns an object with properties that describe the IDS database server"},
    {"get_db_info", (PyCFunction)IfxPy_get_db_info, METH_VARARGS,
     "Returns an object with properties that describe the IDS database server "
     "according to the option passed"},
    {"set_option", (PyCFunction)IfxPy_set_option, METH_VARARGS,
     "Sets the specified option in the resource"},
    {"special_columns", (PyCFunction)IfxPy_special_columns, METH_VARARGS,
     "Returns a result set listing the unique row identifier columns for a "
     "table"},
    {"statistics", (PyCFunction)IfxPy_statistics, METH_VARARGS,
     "Returns a result set listing the index and statistics for a table"},
    {"stmt_error", (PyCFunction)IfxPy_stmt_error, METH_VARARGS,
     "Returns a string containing the SQLSTATE returned by an SQL statement"},
    {"stmt_warn", (PyCFunction)IfxPy_stmt_warn, METH_VARARGS,
     "Returns a warning string containing the SQLSTATE returned by last SQL "
     "statement"},
    {"stmt_errormsg", (PyCFunction)IfxPy_stmt_errormsg, METH_VARARGS,
     "Returns a string containing the last SQL statement error message"},
    {"table_privileges", (PyCFunction)IfxPy_table_privileges, METH_VARARGS,
     "Returns a result set listing the tables and associated privileges in a "
     "database"},
    {"tables", (PyCFunction)IfxPy_tables, METH_VARARGS,
     "Returns a result set listing the tables and associated metadata in a "
     "database"},
    {"get_last_serial_value", (PyCFunction)IfxPy_get_last_serial_value,
     METH_VARARGS, "Returns last serial value inserted for identity column"},
    {"SpeedTestWithCPrimeCount", (PyCFunction)SpeedTestWithCPrimeCount,
     METH_VARARGS,
     "Used for Speed Test C & Python function by counting number of prime "
     "numbers between X and Y"},
#ifdef HAVE_SMARTTRIGGER
    {"open_smart_trigger", (PyCFunction)IfxPy_open_smart_trigger, METH_VARARGS,
     "Open Smart Trigger session"},
    {"get_smart_trigger_session_id",
     (PyCFunction)IfxPy_get_smart_trigger_sessionID, METH_VARARGS,
     "Get already opened Smart Trigger session ID"},
    {"join_smart_trigger_session",
     (PyCFunction)IfxPy_join_smart_trigger_session, METH_VARARGS,
     "Join already opened Smart Trigger session ID"},
    {"register_smart_trigger_loop",
     (PyCFunction)IfxPy_register_smart_trigger_loop, METH_VARARGS,
     "Open Smart Trigger session with loop handled by Python"},
    {"register_smart_trigger_no_loop",
     (PyCFunction)IfxPy_register_smart_trigger_no_loop, METH_VARARGS,
     "Open Smart Trigger session with loop handled by application"},
    {"delete_smart_trigger_session",
     (PyCFunction)IfxPy_delete_smart_trigger_session, METH_VARARGS,
...
static const char DriverModuleDescription[] =
    "Informix Native Driver for Python.";

#if PY_MAJOR_VERSION >= 3
static struct PyModuleDef moduledef = {
    PyModuleDef_HEAD_INIT, DriverModuleName, DriverModuleDescription, -1,
    IfxPy_Methods,
};
#endif

/* Module initialization function */
PyMODINIT_FUNC INIT_IfxPy(void)
{
  PyObject *m;
...
#if PY_MAJOR_VERSION < 3
  // m = Py_InitModule3("IfxPy", IfxPy_Methods, "Informix Native Driver for
  // Python.");
