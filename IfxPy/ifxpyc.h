...
// Note: The below definitions comes from ODBC component of CSDK 4.50.xC1
// release i..e INFORMIXDIR\incl\cli\infxcli.h file. Smart trigger feature is
// supported in Informix CSDK from 4.50.xC1 release onward. The below hard coded
// values are being used to avoid failure of the Python driver build if any
// lower version of CSDK is used. The idea is not to fail the build, however in
// the lower verison (<4.50.xC1) of CSDK at "run time" Smart Trigger will not
// work. Since the implementation for Smart Trigger is from 4.50.xC1 release
// onward.
...
#define PY_IFMX_GET_LO_FILE_DESC_SMART_TRIGGER \
  2286 // SQL_INFX_ATTR_GET_LO_FILE_DESC_SMART_TRIGGER
#define PY_IFMX_GET_SESSION_ID_SMART_TRIGGER \
  2287 // SQL_INFX_ATTR_GET_SESSION_ID_SMART_TRIGGER
#define PY_IFMX_REGISTER_SMART_TRIGGER \
  2288 // SQL_INFX_ATTR_REGISTER_SMART_TRIGGER
#define PY_IFMX_GET_DATA_SMART_TRIGGER_LOOP \
  2289 // SQL_INFX_ATTR_GET_DATA_SMART_TRIGGER_LOOP
#define PY_IFMX_GET_DATA_SMART_TRIGGER_NO_LOOP \
  2290 // SQL_INFX_ATTR_GET_DATA_SMART_TRIGGER_NO_LOOP
#define PY_IFMX_DELETE_SMART_TRIGGER 2291 // SQL_INFX_ATTR_DELETE_SMART_TRIGGER

// Currently, it has provision to register 10 smart triggers. Depending on need
// it could be increased with other relevant code changes in the code.
...
static PyMemberDef le_client_info_members[] = {
    {"DRIVER_NAME", T_OBJECT_EX, offsetof(le_client_info, DRIVER_NAME), 0,
     "Driver Name"},
    {"DRIVER_VER", T_OBJECT_EX, offsetof(le_client_info, DRIVER_VER), 0,
     "Driver Version"},
    {"DATA_SOURCE_NAME", T_OBJECT_EX,
     offsetof(le_client_info, DATA_SOURCE_NAME), 0, "Data Source Name"},
    {"DRIVER_ODBC_VER", T_OBJECT_EX, offsetof(le_client_info, DRIVER_ODBC_VER),
     0, "Driver ODBC Version"},
    {"ODBC_VER", T_OBJECT_EX, offsetof(le_client_info, ODBC_VER), 0,
     "ODBC Version"},
    {"ODBC_SQL_CONFORMANCE", T_OBJECT_EX,
     offsetof(le_client_info, ODBC_SQL_CONFORMANCE), 0, "ODBC SQL Conformance"},
    // {"APPL_CODEPAGE",        T_OBJECT_EX, offsetof(le_client_info,
    // APPL_CODEPAGE),       0, "Application Codepage"},
    // {"CONN_CODEPAGE",        T_OBJECT_EX, offsetof(le_client_info,
    // CONN_CODEPAGE),       0, "Connection Codepage"},
    {NULL} //  Sentinel
};

static PyTypeObject client_infoType = {
    PyVarObject_HEAD_INIT(NULL, 0) "IfxPy.IFXClientInfo", // tp_name
...
static PyMemberDef le_server_info_members[] = {
    {"DBMS_NAME", T_OBJECT_EX, offsetof(le_server_info, DBMS_NAME), 0,
     "Database Server Name"},
    {"DBMS_VER", T_OBJECT_EX, offsetof(le_server_info, DBMS_VER), 0,
     "Database Server Version"},
    // {"DB_CODEPAGE",             T_OBJECT_EX, offsetof(le_server_info,
    // DB_CODEPAGE),             0, "Database Codepage"},
    {"DB_NAME", T_OBJECT_EX, offsetof(le_server_info, DB_NAME), 0,
     "Database Name"},
    {"INST_NAME", T_OBJECT_EX, offsetof(le_server_info, INST_NAME), 0,
     "Database Server Instance Name"},
    {"SPECIAL_CHARS", T_OBJECT_EX, offsetof(le_server_info, SPECIAL_CHARS), 0,
     "Characters that can be used in an identifier"},
    {"KEYWORDS", T_OBJECT_EX, offsetof(le_server_info, KEYWORDS), 0,
     "Reserved words"},
    {"DFT_ISOLATION", T_OBJECT_EX, offsetof(le_server_info, DFT_ISOLATION), 0,
     "Default Server Isolation"},
    {"ISOLATION_OPTION", T_OBJECT_EX,
     offsetof(le_server_info, ISOLATION_OPTION), 0,
     "Supported Isolation Levels "},
    {"SQL_CONFORMANCE", T_OBJECT_EX, offsetof(le_server_info, SQL_CONFORMANCE),
     0, "ANSI/ISO SQL-92 Specification Conformance"},
    {"PROCEDURES", T_OBJECT_EX, offsetof(le_server_info, PROCEDURES), 0,
     "True if CALL statement is supported by database server"},
    {"IDENTIFIER_QUOTE_CHAR", T_OBJECT_EX,
     offsetof(le_server_info, IDENTIFIER_QUOTE_CHAR), 0,
     "Character to quote an identifier"},
    {"LIKE_ESCAPE_CLAUSE", T_OBJECT_EX,
     offsetof(le_server_info, LIKE_ESCAPE_CLAUSE), 0,
     "TRUE if the database server supports the use of % and _ wildcard "
     "characters"},
    {"MAX_COL_NAME_LEN", T_OBJECT_EX,
     offsetof(le_server_info, MAX_COL_NAME_LEN), 0,
     "Maximum length of column name supported by the database server in bytes"},
    {"MAX_IDENTIFIER_LEN", T_OBJECT_EX,
     offsetof(le_server_info, MAX_IDENTIFIER_LEN), 0,
     "Maximum length of an SQL identifier supported by the database server, "
     "expressed in characters"},
    {"MAX_INDEX_SIZE", T_OBJECT_EX, offsetof(le_server_info, MAX_INDEX_SIZE), 0,
     "Maximum size of columns combined in an index supported by the database "
     "server, expressed in bytes"},
    {"MAX_PROC_NAME_LEN", T_OBJECT_EX,
     offsetof(le_server_info, MAX_PROC_NAME_LEN), 0,
     "Maximum length of a procedure name supported by the database server, "
     "expressed in bytes"},
    {"MAX_ROW_SIZE", T_OBJECT_EX, offsetof(le_server_info, MAX_ROW_SIZE), 0,
     "Maximum length of a row in a base table supported by the database "
     "server, expressed in bytes"},
    {"MAX_SCHEMA_NAME_LEN", T_OBJECT_EX,
     offsetof(le_server_info, MAX_SCHEMA_NAME_LEN), 0,
     "Maximum length of a schema name supported by the database server, "
     "expressed in bytes"},
    {"MAX_STATEMENT_LEN", T_OBJECT_EX,
     offsetof(le_server_info, MAX_STATEMENT_LEN), 0,
     "Maximum length of an SQL statement supported by the database server, "
     "expressed in bytes"},
    {"MAX_TABLE_NAME_LEN", T_OBJECT_EX,
     offsetof(le_server_info, MAX_TABLE_NAME_LEN), 0,
     "Maximum length of a table name supported by the database server, "
     "expressed in bytes"},
    {"NON_NULLABLE_COLUMNS", T_OBJECT_EX,
     offsetof(le_server_info, NON_NULLABLE_COLUMNS), 0,
     "Connectionf the database server supports columns that can be defined as "
     "NOT NULL "},
    {NULL} //  Sentinel
};

static PyTypeObject server_infoType = {
    PyVarObject_HEAD_INIT(NULL, 0) "IfxPy.IFXServerInfo", // tp_name
...
typedef struct conn_handle_struct {
  PyObject_HEAD SQLHANDLE henv;
  SQLHANDLE hdbc;
  long auto_commit;
  long c_bin_mode;
  long c_case_mode;
...
static PyTypeObject stmt_handleType = {
    PyVarObject_HEAD_INIT(NULL, 0) "IfxPy.IFXStatement", // tp_name
